				...
				<div class="row">
					...
					<div class="col-sm-9">
						...
						<div class="beta-products-list">
							<h4>Top Products</h4>
							<div class="beta-products-details">
								<p class="pull-left">{{count($phong_khacdanhmuc)}} phòng</p>
								<div class="clearfix"></div>
							</div>
							<div class="row">
                            @foreach($phong_khacdanhmuc as $khac)
							<?php 
								$img_thumb = json_decode($khac->Image,true);
							...
							<div class="col-md-4 col-sm-6">
								<div class="room-item">
									<div class="wrap-img" style="background: url(uploads/images/<?php echo $img_thumb[0]; ?>) center;     background-size: cover;">
										<img src="" class="lazyload img-responsive">
										<div class="category">
											<a href="{{route('chitietphong', $khac->id)}}">{{ $khac->housetype->name }}</a>
										</div>
									</div>
									<div class="room-detail">
										<h3><a href="{{route('chitietphong', $khac->id)}}">{{ $khac->title }}</a></h3>
										<div class="room-meta">
											<span><i class="fas fa-user-circle"></i> Người đăng: <a href="/"> {{ $khac->account->fullname }}</a></span>
										</div>
										<div class="room-info">
											<span><i class="far fa-stop-circle"></i> Diện tích: <b>{{ $khac->size }} m<sup>2</sup></b></span>
											<span class="pull-right">
												<i class="fas fa-eye"></i> Lượt xem: <b>{{ $khac->count_view }}</b>
											</span>
											<br/>
											<span>
												<a class="single-item add-to-cart pull-right" href="{{route('follow', $khac->id)}}"><i class="fa fa-shopping-cart"></i></a>
											</span>
											<div><i class="fas fa-map-marker"></i> Địa chỉ: {{ $khac->address }}</div>
											<div style="color: #e74c3c"><i class="far fa-money-bill-alt"></i> Giá thuê: 
												<b>{{$khac->price}} đồng/{{$khac->pricePer}}</b></div>
											</div>
										</div>
